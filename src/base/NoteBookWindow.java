...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		buttonSave.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				FileChooser fileChooser = new FileChooser();
				fileChooser.setTitle("Please Choose An File");
						...
						if (f.getName().equals(currentFolder)) {
							for (Note n: f.getNotes()) {
								if (n.getTitle().equals(title)) {
									TextNote temp = (TextNote) n;
									String content = temp.getContent();
									textAreaNote.setText(content);
									currentNote = temp.getTitle();
								}
							}
						}
					}
		...
		buttonAddNote.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				if (currentFolder.equals("") || currentFolder.equals("-----")) {
				    ...
				    Alert alert = new Alert(AlertType.INFORMATION);
					alert.setTitle("Successfully saved");
					alert.setContentText("Insert note " + result.get() + " to the folder " + result.get() + " successfully!");
					alert.showAndWait().ifPresent(rs -> {
					    if (rs == ButtonType.OK) {
					        return;
					    }
			        ...
			        if (result.get().equals("")) {
			        	...
			        	for (Folder f:noteBook.getFolders()) {
			        		if (f.getName().equals(target)) {
			        			Alert alert = new Alert(AlertType.WARNING);
								alert.setTitle("Warning");
								alert.setContentText("You already have a folder named with " + target);
								alert.showAndWait().ifPresent(rs -> {
								    if (rs == ButtonType.OK) {
								        return;
								    }
			        	...
			        	noteBook.addFolder(target);
			        	foldersComboBox.getItems().add(target);
			        	currentFolder = target;
			        	foldersComboBox.setValue(target);
			        	updateListView("");
			        }
			    }
			}
	...
	private void updateListView(String keyword) {
		...
		if (keyword.equals("")) {
			...
			for (Folder f: noteBook.getFolders()) {
				if (f.getName().equals(currentFolder)) {
					List<Note> temp = f.searchNotes(keyword);
					for (Note n : temp) {
						list.add(n.getTitle());
					}
				}
					...
					for (Folder f: noteBook.getFolders()) {
						for (Note n: f.getNotes()) {
							if (n.getTitle().equals(currentNote)) {
								int index = f.getNotes().indexOf(n);
								TextNote temp = new TextNote(currentNote, textAreaNote.getText());
								f.getNotes().set(index, temp);
							}
						}
					}
				}
